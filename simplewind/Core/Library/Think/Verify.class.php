...
class Verify {
    ...
    public function entry($id = '') {
        ...
        // 验证码字体随机颜色
        $this->_color = imagecolorallocate($this->_image, mt_rand(1, 80), mt_rand(1, 80), mt_rand(1, 80));
        // 验证码使用随机字体
        ...
        if($this->useZh){ // 中文验证码
            ...
            for ($i = 0; $i<$this->length; $i++) {
                ...
                if($i==0){
                    ...
                    $codeNX += mt_rand($this->fontSize * 1.0, $this->fontSize * 1.3);
                }
                imagettftext($this->_image, $this->fontSize, mt_rand(-20, 20), $codeNX, $this->fontSize*1.3, $this->_color, $this->fontttf, $code[$i]);
            }
        }

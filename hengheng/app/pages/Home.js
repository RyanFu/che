...
export default class HomePage extends Component {
        ...
        <Animated.View style={{
          ...
          <TouchableWithoutFeedback onPress={()=>{}}>
            <View style={[styles.searchBtn, {backgroundColor: "#fff"}]}>
              ...
              <Text style={{fontSize: 13, color:"#666", marginLeft: 5}}>{"输入商品名称"}</Text>
            </View>
          </TouchableWithoutFeedback>
        </Animated.View>
        <Animated.View style={[styles.keywords, {opacity: keyOpaticy}]}>
          {
            ['手工干洗','机器快洗','免费劵','换胎','改装'].map((item, i) => {
              return (
  ...
  _renderTypes(){
    ...
    return (
      <Swiper
        ...
        {renderSwipeView(['洗车','轮胎','钣金','轮胎','钣金','轮胎','钣金','轮胎'], 0)}
        {renderSwipeView(['洗车','轮胎','钣金','轮胎','钣金','轮胎','钣金','轮胎'], 8)}
      </Swiper>
    )
  }
  _renderHot(){
    return ["超值套餐", "免费劵", "抵扣劵", "优惠劵"].map((n, i) => {
      let styl = {
        0: {
          borderBottomWidth: 1,
  ...
  _renderQuality(){
    return (
      <View>
        <View style={{flexDirection: "row", alignItems: "center", justifyContent: "space-between"}}>
          <Text style={{fontSize: px2dp(14), fontWeight: "bold"}}>畅销</Text>
          <TouchableOpacity>
            <View style={{flexDirection: "row", alignItems: "center"}}>
              <Text style={{fontSize: px2dp(12), color: "#aaa", marginRight: 3}}>更多</Text>
              <Icon name="ios-arrow-forward-outline" size={px2dp(13)} color="#bbb" />
        ...
        <View style={{flexDirection: "row", justifyContent:"space-between", alignItems: "center", flexWrap:"wrap", paddingTop: 15}}>
          {
            ["轮胎", "洗车劵", "干洗劵", "机油","轮胎", "洗车劵", "干洗劵", "机油"].map((item, i) => {
      ...
      <View style={{flex: 1, backgroundColor: "#f3f3f3"}}>
        ...
        >
          ...
          <View style={styles.business}>
            <Text style={{color: "#666", paddingLeft: 16, paddingBottom: 6}}>{"推荐门店"}</Text>
            {this._renderBZ()}

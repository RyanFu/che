    ...
    constructor(props) {
        ...
        this.state = {
            ...
            data: {
                ...
                bulletin: "吉首文艺路20号",
                activities: [
                    {key: "减", text: "满199减50，满499减100"},
                    {key: "赠", text: "首次洗车赠抵扣劵两张"}
                ]
            }
        }
    }
                ...
                <View style={{
                    ...
                    <ScrollableTabView page={0} renderTabBar={() => <TabViewBar/>}>
                        <GoodsList ref="goodsList" minus={this.minusItem.bind(this)} lens={this.state.lens}
                                   ...
                                   tabLabel="服务"/>
                        <Comments headHeight={marginTop} tabLabel="评价(1300条)"/>
                    </ScrollableTabView>
                </View>
            </Animated.View>
        )
                            ...
                            return (
                                <View key={i} style={{flexDirection: "row", alignItems: "center", height: px2dp(18)}}>
                                    ...
                                    <Text numberOfLines={1}
                                          style={{fontSize: px2dp(11), marginLeft: 3, color: "#fff"}}>{item.text}</Text>
                                </View>
                            )
                        })
                    }
    ...
    render() {
        ...
        return (
            <View style={{flex: 1, backgroundColor: "#f3f3f3"}}>
                <Animated.Image source={LocalImg.bg} ref={'backgroundImage'} onLoadEnd={this.imageLoaded.bind(this)}
                                style={[
                                    styles.bg,
                                    {
                ...
                <View style={styles.head}>
                    <Animated.View
                        ...
                        <Image source={LocalImg.h0} style={styles.logo}/>
                        <View style={{marginLeft: 14, flex: 1}}>
                            <Text style={{color: "#fff"}}>{data.name}</Text>
                            <TouchableOpacity>
                                <View style={{flexDirection: "row", paddingTop: 8, paddingBottom: 18}}>
                                    {data.fengniao ? (
                                            <Text style={[styles.label2, {marginRight: 5}]}>{"营业时间"}</Text>) : null}
                                    <Text style={{color: "#fff", fontSize: px2dp(12)}}>{`${data.time}`}</Text>
                                </View>
                            </TouchableOpacity>
